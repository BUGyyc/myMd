...
- 战斗内容：动作游戏的技术方案：
  -  client + server 同源代码
  -  通过Timeline，自定义TimelineClip实现多轨道时序控制
  -  ECS 面向数据编程模式
  -  战斗业务相关的 Buff 系统、属性系统
  -  状态同步下的增量更新与AOI
- 工具开发
  -  部分场景编辑器、大量NPC模拟人群，Way-Point管理
  -  Timeline内接入布娃娃系统控制，控制Lerp变化值
- 性能优化
  - 遮挡剔除控制
  - 通过场景编辑器导出的信息，管理场景小物件加载、卸载时机
  - 按照空间划分 System , 适当降低区域的更新频率，客户端
- 渲染相关：
  - 通过PBD算法预研了布料效果，最后应用了MagicCloth
  - 编写了部分Shader，例如 UI进度条的停顿感处理

22-ftx:
- battle:
  - 通过计算动画的关键帧的关节位置，来判断传球的目标点，推算时间
  - 通过弹簧系统控制移速的变化，得到一个起步加速、急停的结果
  - ECS 的方式，实现帧同步，源于OP的快照数据不好确认，不太好做断线重连
  - 动画系统的维护
- tool:
  - 在常规的帧同步查找问题的工具基础之上，另外提供： 
    - 软链接-》开客户端双工程-》双工程通过systemCopyBuff去验证对方的 帧号 + HashCode
    - 正则表达式，自动补充帧同步关键log
- 性能：
  - 大量观众：GPU-Instance,
  - Animancer, 根据情况控制动画 tick 频率
  - 推美术规范-》具体的技术优化
- 渲染：
  - 球员的球衣号码、姓名
